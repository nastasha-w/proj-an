...
Tvals = 10**(2. +  np.arange(1000)/1000.*(9.5-2.))
...
#### compare gadget and C2 kernels

# from kernel_vs.h, kernel_v3.c
def c2kernel_shape(u):
    return np.array((1.0 - u) * (1.0 - u) * (1.0 - u) * (1.0 - u) * (1.0 + 4 * u))

def gdkernel_shape(u):
    u = np.array(u)
    out = np.ones(u.shape) * np.NaN
    temp = u[u < 0.5]
    out[u <  0.5] = 2.546479089470 + 15.278874536822 * (temp - 1) * temp * temp
    temp = u[u >= 0.5]
    out[u >= 0.5] = 5.092958178941 * (1.0 - temp) * (1.0 - temp) * (1.0 - temp)
    return out

class Kernel:
    '''
    Defines kernel functions, normalisations
    
    kernel options: 'C2', 'gadget'
    '''
    def __init__(self, kernel, normdims=2):
        self.kernel = kernel
        if self.kernel == 'C2':
            self.kshape = c2kernel_shape
        elif self.kernel == 'gadget':
            self.kshape = gdkernel_shape
        else:
            raise ValueError('%s is not a valid kernel option'%(self.kernel))
            
        if normdims == 2:
            self.normalize2d()
        else:
            raise NotImplementedError('Only 2d normalisations are supported')
    
    def normalize2d(self, numpoints=1000):
        '''
        normalize the kernel using numpoints for integration so that
        the integrated value over a unit circle area is 1
        '''
        self.sample = np.arange(0., 1. + 0.5 / float(numpoints), 1. / float(numpoints))
        self.norm = np.sum(2. * np.pi * np.average(np.diff(self.sample)) * self.sample * self.kshape(self.sample))
        self.norm = 1. / self.norm
        
    def __call__(self, u):
        '''
        returns normalized kernel
        u = length normalized to kernel support size
        '''
        u = np.array(u)
        self.out = self.norm * self.kshape(u)
        print(u)
        print(self.out)
        self.out[u > 1] = 0.
        return self.out