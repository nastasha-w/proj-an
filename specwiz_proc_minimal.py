...
def comparetxt(name1,name2):
    ...
    if lines1 != lines2:
        print('Files are of different length:\n %s has %i\n %s has %i'%s(name1,lines1,name2,lines2))
...
#            if dions == 'all':
#                dions = self.ions
#            self.coldens.update({ion: np.array([self.specfile['Spectrum%i/%s/LogTotalIonColumnDensity'%(specnum,ion)].value for specnum in range(self.numspecs)]) for ion in dions})
...
#        if realspace:
#            try:
#                self.nion.update({ion: np.array([self.specfile['Spectrum%i/%s/RealSpaceNionWeighted/NIon_CM3'%(specnum,ion)].value for specnum in range(self.numspecs)]) for ion in dions})
