...
Fixed it so .tar files are ignored
July 10, 2015 Chris Barber added a correction for some variables that
had incorrect values for aexp-scale-factor in the original EAGLE runs
"""
...
filename_options = ['snap', 'snip', 'group', 'group_snip', 'sub', 'sub_snip', 'particles', 'particles_snip']

class read_eagle_file:
    def __init__( self, model_dir, filename, snapnum, suppress=True, gadgetunits=False, subsample_factor=1, ioneq_str=None, add_data_dir = True):
        ...
        self.fname = self.get_full_dir(model_dir, filename, snapnum, ioneq_str = ioneq_str, add_data_dir = add_data_dir)
    ...
    def get_full_dir(self, model_dir, filename, snapnum, ioneq_str = None, add_data_dir = True):
        """Get filename, including full path"""
        if ioneq_str is None:
            ioneq_str = ''
        if add_data_dir:
        ...
        if filename == 'snap':
            ...
            try:
                dirname = "snapshot_%s%.3i_z***p***" % (ioneq_str, snapnum)
                _dir = fnmatch.filter( next(os.walk(dirpath))[1], dirname)[0]
                string = _dir.rsplit('_')
                fname = dirpath + _dir + "/snap_%s%.3i_%s." % (ioneq_str, snapnum, string[-1])
            #otherwise use old format
            except:
                dirname = "snapshot_%s%.3i/" % (ioneq_str, snapnum)
                fname = "/snap_%s%.3i." % (ioneq_str, snapnum)
        elif filename == 'snip':
            #check to see if using new file format
            try:
                ...
                _dir = fnmatch.filter( next(os.walk(dirpath))[1], dirname)[0]
                string = _dir.rsplit('_')
                fname = dirpath + _dir + "/snip_%.3i_%s." % (snapnum, string[-1])
            #otherwise use old format
            except:
            ...
            try:
                ...
                _dir = fnmatch.filter( next(os.walk(dirpath))[1], dirname)[0]
                string = _dir.rsplit('_')
                fname = dirpath + _dir + "/eagle_subfind_particles_%.3i_%s." % (snapnum, string[-1])
            except:
                _dir = "particledata_%.3i/" % snapnum
                fname = dirpath + _dir +"eagle_subfind_particles_%.3i." % snapnum
        elif filename == 'particles_snip':
            try:
                dirname = "particledata_snip_%.3i_z***p***" % snapnum
                _dir = fnmatch.filter( next(os.walk(dirpath))[1], dirname)[0]
                string = _dir.rsplit('_')
                fname = dirpath + _dir + "/eagle_subfind_snip_particles_%.3i_%s." % (snapnum, string[-1])
            except:
                _dir = "particledata_%.3i/" % snapnum
                fname = dirpath + _dir +"eagle_subfind_snip_particles_%.3i." % snapnum

        #snap subfind/FOF halo data:
        elif filename == 'group':
            try:
                dirname = "groups_%.3i_z***p***" % snapnum
                _dir = fnmatch.filter( next(os.walk(dirpath))[1], dirname)[0]
                string = _dir.rsplit('_')
                fname = dirpath + _dir + "/group_tab_%.3i_%s." % (snapnum, string[-1])
            except:
                dirname = "groups_%.3i/" % snapnum
                fname = dirpath + _dir + "/sub_%.3i." % (snapnum)
        elif filename == 'sub':
            try:
                dirname = "groups_%.3i_z***p***" % snapnum
                _dir = fnmatch.filter( next(os.walk(dirpath))[1], dirname)[0]
                string = _dir.rsplit('_')
                fname = dirpath + _dir + "/eagle_subfind_tab_%.3i_%s." % (snapnum, string[-1])
            except:
                dirname = "groups_%.3i/" % snapnum
                fname = dirpath + _dir + "/sub_%.3i." % (snapnum)

        # snip subfind/FOF halo data:
        elif filename == 'group_snip':
            try:
                dirname = "groups_snip_%.3i_z***p***" % snapnum
                _dir = fnmatch.filter( next(os.walk(dirpath))[1], dirname)[0]
                string = _dir.rsplit('_')
                fname = dirpath + _dir + "/group_snip_tab_%.3i_%s." % (snapnum, string[-1])
            except:
                dirname = "groups_snip_%.3i/" % snapnum
                fname = dirpath + _dir + "/sub_%.3i." % (snapnum)
        elif filename == 'sub_snip':
            try:
                dirname = "groups_snip_%.3i_z***p***" % snapnum
                _dir = fnmatch.filter( next(os.walk(dirpath))[1], dirname)[0]
                string = _dir.rsplit('_')
                fname = dirpath + _dir + "/eagle_subfind_snip_tab_%.3i_%s." % (snapnum, string[-1])
            except:
                dirname = "groups_snip_%.3i/" % snapnum
                fname = dirpath + _dir + "/sub_%.3i." % (snapnum)
#
#        else:
#            try:
#                if filename == 'group_snip' or filename == 'sub_snip':
#                    dirname = "groups_snip_%.3i_z***p***" % snapnum
#                else:
#                    dirname = "groups_%.3i_z***p***" % snapnum
...
#                string = dir.rsplit('_')
#
#                if filename == 'group_snip' or filename =='group':
#                    fname = dirpath + dir + "/group_tab_%.3i_%s." % (snapnum, string[-1])
#                elif filename == 'sub_snip' or filename == 'sub':
...
#                        fname = dirpath + dir + "/sub_%.3i." % (snapnum)
        ...
        try:
            ...
            print("[read_eagle_file] ERROR: File %s does not exist/can not be opened"%fname)
            print("exiting....")
            exit(-2)

        self.NumFiles = f['Header'].attrs['NumFilesPerSnapshot']
        if filename == 'snap' or filename=='snip' or filename == 'particles' or filename == 'particles_snip':
            ...
            try:
                ...
                if filename == 'sub' or filename == 'sub_snip':
    ...
    def read_data_array(self, var, gadgetunits=False, suppress=False, interval=1, region=None, requiredlength=1e9):
        ...
        #note also that MetalMassWeightedRedshift were incorrectly computed in all runs prior to the revision 28341 (16/06/2014) of the code and that Element Abundances and Smoothed Element Abundances are wrong for a small fraction of haloes in some of the simulations
        ...
        #if self.filename == 'snap' or self.filename == 'snip' or self.filename == 'particles' or self.filename == 'particles_snip':
        if self.filename in ('snap', 'snip', 'particles', 'particles_snip'):
            ...
            try:
                ...
                #let's add in a line where we correct for the bug in the aexp-scale-exponent for various variables (See Eagle wiki for explanation), added by Chris Barber July 10, 2015
                if var in ('Subhalo/HalfMassRad','Subhalo/HalfMassRadProj','Subhalo/InertiaTensor','Subhalo/StellarVelDisp', 'Subhalo/StellarVelDisp_HalfMassProjRad','Subhalo/ApertureMeasurements/VelDisp'):
                    self.a_scaling = 0.0

                #also the FOF velocties were wrong:
                if var == 'FOF/Velocities':
                    self.a_scaling = -1.0
           ...
           #close file opened for reading attributes
            ...
            if self.NumPartTot[parttype] == 0:
                print("No particles of type %s in snapshot %s"%(parttype, self.snap))
                return []
            ...
            while read :
                ...
                if Ndata == 0:
                    if not suppress: print("Num part in file %i is %i continuing..."%(j, Ndata))
                    f.close()
                    j = j + 1
                    if j >= self.NumFiles :
...
#        self.test_array(var)
    ...
    def read_var(self, f, var, j, suppress, interval=1):
        ...
        try:
            ...
            print('[read_eagle_file] ERROR: Variable %s not found, file %i'%(var, j))
            return False
    ...
    def read_nohashtable_array(self, var, gadgetunits=False, suppress=False, interval=1):
        ...
        while _len == 0 :
            ...
            if self.filename in ('snap','snip','particles','particles_snip'):
                ...
                _len = NumPartFile[parttype]
            else:
                if 'ParticleIDs' in var:
                    _len = f['Header'].attrs['Nids']
                elif 'FOF' in var:
                    _len = f['Header'].attrs['Ngroups']
                elif 'Subhalo' in var:
                    _len = f['Header'].attrs['Nsubgroups']
            if _len == 0:
                if not suppress: print("Num part in file %i is %i continuing..."%(j, _len))
                f.close()
                j = j + 1
                if j >= self.NumFiles :
                    print("No particles found in any file!!!")
    ...
    def test_array(self, var):
        ...
        if 'ParticleIDs' not in var:
            if data_len != self.NumPartTot[parttype] and self.subsample_factor == 1:
                print('[read_eagle_file] ERROR: Length of %s should be %i array of length %i read in!'%(var, self.NumPartTot[parttype], data_len))
...
if __name__ == "__main__":
    _dir = '/gpfs/data/Eagle/mfTestRuns/CodeDev/SubFind/AddVariables'
    snap = 28

    print('Testing groupfile')
    f = read_eagle_file(_dir, 'group', snap)
    ...
    f.close()
    read = read_eagle_file(_dir, 'snap', snap)
    start=time.time()
