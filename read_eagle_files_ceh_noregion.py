...
import pprint
#import read_eagle
import os # for searching directories for z***p*** match
...
class Gadget(object):
    ...
    def __init__(self, model_dir, file_type, snapnum, sim='BAHAMAS',
                 smooth=False,verbose=False, gadgetunits=False, ioneq_str=None, **kwargs):
        '''
        ...
        if sim not in sims:
            ...
            self.sim = sim
        self.filename = self.get_full_dir(model_dir, file_type, snapnum, sim, ioneq_str=ioneq_str)
    ...
    # ==========================================================================
    def get_full_dir(self, model_dir, file_type, snapnum, sim, ioneq_str=None):
        '''
        ...
        elif sim == 'EAGLE-IONEQ':
            dirpath = dirpath[:-5] # no data subdirectory here -> remove from directory path
            if file_type == 'snap':
                dirname = "snapshot_%s%.3i_z***p***" % (ioneq_str, snapnum)
                _dir = fnmatch.filter( next(os.walk(dirpath))[1], dirname)[0]
                string = _dir.rsplit('_')
                dirname = _dir
                fname = "/snap_%s%.3i_%s." % (ioneq_str, snapnum, string[-1])
        elif sim == 'C-EAGLE':
            if file_type == 'snap':
                ...
                string = _dir.rsplit('_')
                dirname = _dir
                fname = "/snap_%.3i_%s." % (snapnum, string[-1])
            elif file_type == 'particles':
                dirname = "particledata_%.3i_z***p***"%(snapnum)
                _dir = fnmatch.filter( next(os.walk(dirpath))[1], dirname)[0]
                string = _dir.rsplit('_')
                dirname = _dir
                fname = "/eagle_subfind_particles_%.3i_%s." % (snapnum, string[-1])
            else:
        ...
        elif sim == 'BAHAMAS' or sim == 'C-EAGLE':
            # Read in file and particle info
            self.num_files     = f['Header'].attrs['NumFilesPerSnapshot']
            self.num_part_tot  = f['Header'].attrs['NumPart_Total']
            self.num_part_file = f['Header'].attrs['NumPart_ThisFile']
    ...
    def read_var(self, var, gadgetunits=False, verbose=True):
        ...
        if self.sim == 'OWLS':
            self.data = self.read_owls_array(var,
                                             ...
                                             verbose=verbose)
        elif self.sim == 'BAHAMAS' or self.sim == 'C-EAGLE':
            self.data = self.read_bahamas_array(var,
                                                gadgetunits=gadgetunits,
                                                verbose=verbose)
        elif self.sim == 'EAGLE-IONEQ':
            self.data = self.read_bahamas_array(var,
                                                gadgetunits=gadgetunits,
                                                verbose=verbose)
